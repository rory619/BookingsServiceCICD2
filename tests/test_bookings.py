...
def create_booking_ok(client, payload=None):
    ...
    with patch("app.main.check_user_with_circuit_breaker", return_value=(True, "checked")):
...
def test_create_booking_user_not_found(client):
    #  404
    with patch("app.main.check_user_with_circuit_breaker", return_value=(False, "checked")):
        r = client.post("/api/bookings", json={"user_id": 999, "course_id": 10, "status": "confirmed"})
    assert r.status_code == 404


def test_create_booking_users_down_sets_pending(client):
    # circuit breaker open
    with patch("app.main.check_user_with_circuit_breaker", return_value=(None, "skipped_users_down")):
        r = client.post("/api/bookings", json={"user_id": 1, "course_id": 10, "status": "confirmed"})
    assert r.status_code == 201
...
def test_proxy_greet(client):
    ...
    # Patch the Client constructor used inside your endpoint (not .get on the class)
...
def test_publish_order(client):
    ...
    with patch("app.main.aio_pika.connect_robust", new=AsyncMock(return_value=fake_conn)):
        with patch("app.main.aio_pika.Message", return_value=MagicMock()):
